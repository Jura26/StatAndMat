   ...
   async (req, res) => {
      try {
         if (req.user) {
            ...
            console.error(
               "Authentication failed: req.user is undefined or login denied."
            );
...
const smtpSecure = smtpPort === 465; // true for SMTPS (port 465)

const transporter = nodemailer.createTransport({
   host: process.env.EMAIL_HOST,
   port: smtpPort,
   secure: smtpSecure,
   requireTLS: !smtpSecure, // enable STARTTLS for ports like 587
   auth: {
...
app.post("/resend-email", async (req, res) => {
   try {
      ...
      User.findOne({ email: email }).then((present) => {
         ...
         // Generate email token
         const emailToken = jwt.sign({ username }, process.env.EMAIL_SECRET, {
            expiresIn: "1d",
         });
...
app.post("/api/login", async (req, res) => {
   try {
      ...
      if (!user) {
         return res
            .status(401)
            .json({ error: "Incorrect username or email address" });
      }
      ...
      if (!user.isConfirmed) {
         return res
            .status(401)
            .json({ error: "Confirm your email address to continue" });
      }
      ...
      req.sessionStore.destroy(req.sessionID, (storeErr) => {
         if (storeErr) {
            return res.status(500).json({
               error: "Could not log out and delete session from database",
            });
         }
      ...
      <body>
         <div class="email-container">
            ...
            <div class="content">
               ...
               <p>Dear customer,</p>
               <p>We appreciate your order. You have successfully purchased <strong>"Mathematics - First Exam"</strong> for the account <strong>${name}</strong>.</p>
               <p>We wish you success in your studies and hope the material helps you achieve your goals.</p>
               <p>If you have any questions, feel free to contact us by email.</p>
               <p>Best regards,</p>
               <p><strong>Stat&Mat Team</strong></p>
            </div>
            <div class="footer">
   ...
   <body>
      <div class="email-container">
         ...
         <div class="content">
            <h1>Thank you for your purchase!</h1>
            <p>Dear customer,</p>
            <p>We appreciate your order. You have successfully purchased <strong>"Mathematics - Second Exam"</strong> for the account <strong>${name}</strong>.</p>
            <p>We wish you success in your studies and hope the material helps you achieve your goals.</p>
            <p>If you have any questions, feel free to contact us by email.</p>
            <p>Best regards,</p>
            <p><strong>Stat&Mat Team</strong></p>
...
app.get("/proxy/1video", async (req, res) => {
   try {
      ...
      if (Param == "1.2.1") {
         videoUrl =
            "https://iframe.mediadelivery.net/embed/368157/5eee48d7-6981-461f-9185-1d51c8f34764?autoplay=false&loop=false&muted=false&preload=true&responsive=true";
      }
      if (Param == "1.2.2") {
         videoUrl =
            "https://iframe.mediadelivery.net/embed/368157/15ee6ea2-83b9-474a-a658-fd4920548cbc?autoplay=false&loop=false&muted=false&preload=true&responsive=true";
      }
...
app.get("/statistics1", (req, res) => {
   res.render("statistics1.ejs");
});
app.get("/statistics2", (req, res) => {
   res.render("statistics2.ejs");
});
...
app.get("/math-first-exam", requireSubscription1, (req, res) => {
   res.sendFile(path.join(__dirname, "public", "mathFirstExam.html"));
});

app.get("/math-second-exam", requireSubscription2, (req, res) => {
   res.sendFile(path.join(__dirname, "public", "mathSecondExam.html"));
});
